...
export default function InventarioMasasPage() {
  ...
  const [selectedMasa, setSelectedMasa] = useState<MonoHiloMasa | null>(null)
  ...
  return (
    <AdminShell rightPanel={rightPanel}>
      <div className="space-y-8">
              ...
              <table className="min-w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-500">
                    ...
                    <th className="px-2 py-2 text-center">Detalles</th>
        ...
        <Dialog open={!!selectedMasa} onOpenChange={() => setSelectedMasa(null)}>
          <DialogContent className="flex h-screen max-h-screen w-screen max-w-full flex-col overflow-hidden rounded-none p-0 sm:rounded-none">
            {selectedMasa && (() => {
              const dimensiones = Object.keys(selectedMasa.estimados)
              return (
                <>
                  <DialogHeader className="shrink-0 border-b border-slate-200 px-6 py-4">
                    <DialogTitle className="text-xl">{selectedMasa.codigo}</DialogTitle>
                  </DialogHeader>

                  <div className="flex-1 overflow-y-auto px-6 py-6">
                  <div className="space-y-6 text-sm">
                    <div>
                      <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-500">Datos generales</p>
                      <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                        <div>
                          <p className="text-muted-foreground">Bloque</p>
                          <p className="font-medium">{resolveBlockCode(selectedMasa)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Fecha de registro</p>
                          <p className="font-medium">{formatDate(selectedMasa.fechaRegistro)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Ubicacion</p>
                          <Badge variant="outline" className="mt-0.5 border-slate-200 bg-slate-50 text-slate-700">
                            {selectedMasa.ubicacion}
                          </Badge>
                        </div>
                        <div>
                          ...
                          <Badge
                            variant="outline"
                            className={`mt-0.5 ${selectedMasa.estado === 'anulada' ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700'}`}
                          >
                            {selectedMasa.estado ?? 'activa'}
                          </Badge>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-500">Dimensiones y corte</p>
                      <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                        <div>
                          <p className="text-muted-foreground">Largo (cm)</p>
                          <p className="font-medium">{selectedMasa.largoCm.toFixed(2)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Ancho (cm)</p>
                          <p className="font-medium">{selectedMasa.anchoCm.toFixed(2)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Profundidad (cm)</p>
                          <p className="font-medium">{selectedMasa.profundidadCm.toFixed(2)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Margen (cm)</p>
                          <p className="font-medium">{selectedMasa.margenCm.toFixed(2)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Grosor disco (mm)</p>
                          <p className="font-medium">{selectedMasa.grosorDiscoMm}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Espesor losa (cm)</p>
                          <p className="font-medium">{selectedMasa.espesorLosaCm}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    {dimensiones.length > 0 && (
                      <div>
                        <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-500">Estimados por dimension</p>
                        <div className="space-y-3">
                          {dimensiones.map((dim) => {
                            const est = selectedMasa.estimados[dim]
                            const disponibles = getMonoHiloLosasDisponibles(selectedMasa, dim)
                            return (
                              <div key={dim} className="rounded-lg border border-slate-200 bg-slate-50/70 p-3">
                                <p className="mb-2 font-semibold text-slate-800">{dim}</p>
                                <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs">
                                  <div>
                                    <p className="text-muted-foreground">Estimadas</p>
                                    <p className="font-medium">{est?.losasEstimadas ?? 0}</p>
                                  </div>
                                  <div>
                                    <p className="text-muted-foreground">Consumidas</p>
                                    <p className="font-medium">{est?.losasConsumidas ?? 0}</p>
                                  </div>
                                  <div>
                                    <p className="text-muted-foreground">Disponibles</p>
                                    <p className="font-medium text-emerald-700">{disponibles}</p>
                                  </div>
                                  <div>
                                    <p className="text-muted-foreground">Merma estimada</p>
                                    <p className="font-medium">
                                      {est?.mermaEstimadaM3?.toFixed(3) ?? '0.000'} m³
                                      {est?.mermaEstimadaPorcentaje != null
                                        ? ` (${est.mermaEstimadaPorcentaje.toFixed(1)}%)`
                                        : ''}
                                    </p>
                                  </div>
                                </div>
...
{selectedMasa.observaciones && (
                      <div>
                        <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-slate-500">Observaciones</p>
                        <p className="rounded-lg border border-slate-200 bg-slate-50/70 p-3 text-xs text-slate-700">{selectedMasa.observaciones}</p>
                      </div>
                    )}
                  </div>
                  </div>
